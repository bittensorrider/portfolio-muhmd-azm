      ...
      {items.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((it, idx) => (
            <div
              key={`${it.name}-${idx}`}
              className="pf-card group relative overflow-hidden rounded-2xl border border-subtle bg-surface focus-within:ring-2 focus-within:ring-(--accent)/50"
            >
              <button
                type="button"
                onClick={() => open(idx)}
              ...
              <div className="flex items-start justify-between gap-3 p-4">
                ...
                {it.link && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="shrink-0 rounded-lg border border-subtle bg-(--bg)/60 px-2 py-1 text-xs hover:bg-(--accent)/10 hover:text-(--accent) transition"
            ...
            >
              <div ref={swipeRef} className="pf-stage">
                ...
                <figcaption className="pf-caption">
                  <div className="min-w-0">
                    <strong className="block text-(--text)">{items[active].name}</strong>
