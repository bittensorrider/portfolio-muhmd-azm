...
export const Resume: React.FC<{ resume: ResumeData }> = ({ resume }) => {
      ...
      <ul role="list" aria-labelledby={labelId} className="relative ml-3 border-l border-subtle">
        {items.map((e, i) => (
          <li key={i} className="relative pl-6 py-4">
            <span
              aria-hidden="true"
              className="absolute left-[-7px] top-5 h-2.5 w-2.5 rounded-full bg-(--accent) ring-2 ring-(--bg)"
            />
            ...
            {(e.period || e.details) && (
              <div className="mt-1 text-sm">
                {e.period && (
                  <span className="mr-2 rounded-md border border-subtle bg-(--bg) px-2 py-0.5 font-mono text-xs">
                    {e.period}
                  </span>
                )}
                {e.details && <p className="mt-2 leading-relaxed text-(--text)/90">{e.details}</p>}
              </div>
            )}
          </li>
        ))}
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="rounded-2xl border border-subtle bg-(--bg)/55 backdrop-blur-xl shadow-lg p-6 sm:p-8">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <article className="rounded-2xl border border-subtle bg-(--bg)/55 backdrop-blur-xl shadow-lg p-6 sm:p-8">
