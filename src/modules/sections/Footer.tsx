...
export const Footer: React.FC<{ socials?: SocialsInput; name?: string; studioTag?: string }> = ({
  ...
  return (
    <footer className="relative mt-24 text-sm">
      ...
      <div className="w-full border-t border-subtle bg-(--bg)/65 backdrop-blur-xl">
          ...
          <div className="mx-auto grid max-w-3xl grid-cols-[repeat(auto-fit,minmax(44px,1fr))] gap-3">
            {pairs.map(([key, url]) => (
              <a
                ...
                aria-label={key}
                title={key}
                className="group inline-flex items-center justify-center rounded-xl p-2 border border-subtle bg-(--bg)/55 hover:bg-(--accent)/10 transition shadow-sm hover:shadow focus-ring"
              >
            ...
            {custom.map((c) => {
              ...
              return (
                <a
                  ...
                  rel="noreferrer"
                  title={c.label}
                  className="group col-span-2 sm:col-span-1 inline-flex items-center gap-2 rounded-xl px-3 py-2 border border-subtle bg-(--bg)/55 hover:bg-(--accent)/10 transition shadow-sm hover:shadow focus-ring"
                >
                  {c.iconUrl ? (
                    <img
                      src={c.iconUrl}
                    ...
                    <span className="text-(--accent)">{pickIcon("website")}</span>
                  )}
                  <span className="truncate text-(--text) group-hover:text-(--accent) text-xs font-medium">
                    {c.label}
                  </span>
                </a>
              );
